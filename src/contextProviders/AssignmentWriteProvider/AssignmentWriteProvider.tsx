...
const AssignmentWriteContext = createContext<AssignmentWriteContextProps>({} as AssignmentWriteContextProps);
...
export const AssignmentWriteProvider: React.FC<Wrapper> = ({ children }) => {
    ...
    const [isChangeTranslatorOpen, setIsChangeTranslatorOpen] = useState<boolean>(false);

    const [isChangeClaimantOpen, setIsChangeClaimantOpen] = useState<boolean>(false);

    const [isChangeAddressOpen, setIsChangeAddressOpen] = useState<boolean>(false);

    const [updateAssignment] = useUpdateAssignmentMutation();
    const { enqueueSnackbar } = useSnackbar();

    const handleUpdateAssignment = (key: keyof UpdateAssignment, data: UpdateAssignment[keyof UpdateAssignment]) => {
    ...
    return (
        <>
            ...
            <Dialog open={isChangeTranslatorOpen} onClose={() => setIsChangeTranslatorOpen(false)} fullWidth maxWidth='md'>
                <DialogContent>
                    <TranslatorSelect
            ...
            <Dialog open={isChangeClaimantOpen} onClose={() => setIsChangeClaimantOpen(false)} fullWidth maxWidth='md'>
                <DialogTitle>
            ...
            <Dialog open={isChangeAddressOpen} onClose={() => setIsChangeAddressOpen(false)} fullWidth maxWidth='md'>
                <DialogTitle>
                    <Typography textAlign='center'>Change Address</Typography>
                </DialogTitle>
                <DialogContent>
                    <AddEditAddressForm mode='edit' onSuccess={(_) => {}} id={assignment && assignment.address ? assignment.address.id : ''} />
                </DialogContent>
            </Dialog>
        </>
    );
...
export const useAssignmentWrite = () => {
    ...
    if (context === undefined) {
        throw new Error('useAssignmentWrite must be used within a AssignmentWriteProvider');
    }
