...
const LanguagesContext = createContext<languagesProviderContext>({} as languagesProviderContext);
...
export const LanguagesProvider: React.FC<{
    ...
    const languageCodes = useMemo(() => languages.map(({ code }) => code), [languages]);

    const getLanguageFromCode = (code: string) => {
        return languagesObject[code] || '';
    };

    return <LanguagesContext.Provider value={{ languages, languagesObject, languageCodes, getLanguageFromCode }}>{children}</LanguagesContext.Provider>;
};

export const useLanguages = () => {
    const data = useContext(LanguagesContext);
    if (!data) throw new Error('Must be in a languagesProvider to use Languages Context');
