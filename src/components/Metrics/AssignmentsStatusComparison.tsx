...
export const AssignmentsStatusComparison: React.FC = () => {
    ...
    const { data, isError, isLoading, isEmpty } = useAssignmentStatusComparison(timePeriod);

    const completedAssignmentsData = data.find((datum) => datum.id === 'Total');

    const maxValue = completedAssignmentsData && completedAssignmentsData.data.length ? max(completedAssignmentsData.data, (d) => d.y) || 1 : 1;

    const tickInterval = Math.ceil(maxValue / 10);

    return (
        <MetricLoadingStates isError={isError} isLoading={isLoading} isEmpty={isEmpty}>
            <Stack direction='column' spacing={2} width='100%' height='100%' alignItems='center'>
                <RangeSelectTabs onChange={setTimePeriod} options={['1 Month', '3 Month', '6 Month', '1 Year', 'All']} />
                <ResponsiveLine
                    data={data || []}
