...
export const ClaimantFilter: React.FC<ClaimantFilterProps> = ({
    ...
    const claimants = assignments.map((assignment) => assignment.claimant).filter((claimant) => claimant !== undefined && claimant !== null) as Claimant[];
            ...
            <Controller
                ...
                render={({ field: { onChange, ref, value } }) => (
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} fullWidth label='Claimant' variant='outlined' error={!!errors.data} helperText={errors.data?.message} />
                        )}
                    />
                )}
            />
