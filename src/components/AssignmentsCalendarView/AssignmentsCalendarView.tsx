...
export const AssignmentsCalendarView: React.FC = () => {
  ...
  const [openEditAssignmentForm, setOpenEditAssignmentForm] =
    useState<boolean>(false);
  ...
  const events = assignments.map((assignment) => {
    ...
    const claimantPrimaryLanguage = getLanguageFromCode(
  ...
  return (
    <>
          ...
          <>
            <Calendar
              ...
              eventPropGetter={(event) => {
                ...
                const { claimantNoShow, translatorNoShow, isComplete } =
                  assignment;
                const style = {
      ...
      <Dialog
        open={openEditAssignmentForm}
        onClose={() => setOpenEditAssignmentForm(false)}
        maxWidth="md"
        fullWidth
      >
        ...
        <DialogContent>
          {selectedAssignment ? (
            <EditCalendarAssignment
              assignment={selectedAssignment}
              onSuccess={() => setOpenEditAssignmentForm(false)}
            />
          ) : null}
        </DialogContent>
      </Dialog>
