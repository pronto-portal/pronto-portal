...
export const AddAssignmentsForm: React.FC<AddAssignmentsFormProps> = ({ open, handleClose }) => {
    ...
    const { reset, date, setDate, claimant, translator, address, setReminder, setConfigureReminderSchedule } = useAddAssignmentFlow();
                    ...
                    {activeStep === 4 && (
                        <ReminderForm
                            onSuccess={(data) => {
                                if (data) {
                                    setReminder(data);
                                    setConfigureReminderSchedule(data.configureReminderSchedule);
                                }
                                setActiveStep(5);
                            }}
                            claimant={claimant}
