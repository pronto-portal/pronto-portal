...
export const AddressFilter: React.FC<AddressFilterProps> = ({
    ...
    const addresses = data && data.getAddresses ? data.getAddresses.addresses : ([] as Address[]);

    const onFormSubmit: SubmitHandler<{ data: Partial<Address> }> = ({ data }) => {
                ...
                <Controller
                    ...
                    render={({ field: { onChange, ref, value } }) => (
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField {...params} label='Address' variant='outlined' error={!!errors.data} fullWidth helperText={errors.data?.message} />
                            )}
                        />
                    )}
                />
